    ...
    <div className="container mx-auto p-6 space-y-4">
      <Breadcrumb>
        <BreadcrumbList>
          ...
          <BreadcrumbSeparator />
          <BreadcrumbItem>
            <BreadcrumbPage>Codes</BreadcrumbPage>
          </BreadcrumbItem>
        </BreadcrumbList>
      </Breadcrumb>

      <h1 className="text-xl font-extrabold uppercase tracking-wide">Codes</h1>
      ...
      <Table>
        ...
        <TableBody>
          {highlights.map(h => (
            <TableRow key={h.id}>
              ...
              <TableCell className="text-right">
                <Button size="sm" variant="destructive" className="rounded-none h-7 px-2" onClick={async () => { if (!confirm('Delete this code?')) return; await deleteHighlight(h.id); qc.invalidateQueries({ queryKey: ['highlights', projectId] }); }}>
                  Delete
                </Button>
              </TableCell>
            </TableRow>
          ...
          {highlights.length === 0 && (
            <TableRow>
              <TableCell colSpan={4} className="text-sm text-neutral-600">No codes yet.</TableCell>
            </TableRow>
          )}
        </TableBody>
      </Table>
